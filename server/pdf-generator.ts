...
function buildHTMLEnd(data: ItineraryFormData): string {
  const contactInfo = [data.companyWebsite, data.companyEmail, data.companyPhone].filter(Boolean).map(escapeHtml).join(" | ");
  let html = "<footer><p>Document generated by <strong>" + escapeHtml(data.companyName) + "</strong></p>";
  html += "<div class=\"company-contact\">" + contactInfo + "</div></footer></div></body></html>";
  return html;
}
